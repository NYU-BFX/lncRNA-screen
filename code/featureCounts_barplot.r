...
usage = "\
        Rscript align_barplot.r [OUT DIR] [featureCounts_summary.txt]
"

align_reads <- function(x){
  x = x %>% filter(V2!="Uniquely_Mapped") %>% mutate(V4=ifelse(grepl("Assigned|Unassigned",V2, ignore.case=T),"Uniquely_Mapped",as.character(V2))) %>% arrange(desc(V1)) %>% mutate(V3=V3/1000000)
  x$V2 <- factor(x$V2, levels = c("Assigned","Unassigned","Multi_Mapped","Unmapped"))
...
align_PCT <- function(x){
  x = x %>% filter(V2!="Uniquely_Mapped") %>% group_by(V1,add=F) %>% mutate(V3=V3/sum(V3)) %>% mutate(V4=ifelse(grepl("Assigned|Unassigned",V2, ignore.case=T),"Uniquely_Mapped",as.character(V2))) %>% arrange(desc(V1)) %>% mutate(V3=V3*100)
  x$V2 <- factor(x$V2, levels = c("Assigned","Unassigned","Multi_Mapped","Unmapped"))
  x$V4 <- factor(x$V4, levels = c("Uniquely_Mapped","Multi_Mapped","Unmapped"))
  x=x[order(x$V1,x$V2),]
